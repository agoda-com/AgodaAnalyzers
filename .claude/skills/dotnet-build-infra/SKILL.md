---
name: dotnet-build-infra
description: Use when editing `.csproj`, `.nuspec`, `Directory.Build.props`, `Directory.Build.targets`, `Directory.Packages.props`, build scripts, or CI workflows in a .NET repository. Covers central package management, version-derivation tooling, the props-vs-targets split, and nuspec/csproj consistency.
---

# .NET build infrastructure

Library builds are opinionated. Several patterns that look like ordinary csproj/nuspec editing in greenfield projects are wrong in a repo that uses central package management, automated versioning, and shared MSBuild files. When editing any build file, follow these conventions.

## Adding a package dependency

If the repo uses **Central Package Management** (`Directory.Packages.props` with `ManagePackageVersionsCentrally`), two files always change in lockstep:
...
Never put `Version="..."` on a `<PackageReference>` in a CPM-enabled repo. If you see one in a diff (often left by an IDE after a package update), delete the attribute.
...
In the corresponding nuspec entry, mark such packages as `developmentDependency="true"` so consumers of the shipped nuget don't pull them transitively.

## Versioning — don't touch it

When versioning is handled at build time by a tool like **MinVer**, **Nerdbank.GitVersioning**, or **GitVersion**, the version is derived from git tags or branch history. Specifically:
...
- A common deliberate convention is that `AssemblyVersion` stays at `Major.0.0.0` (so binding redirects don't churn on every release) while `AssemblyFileVersion` drifts. Don't "fix" this if it's the existing pattern.

If you need to influence the version (e.g. for a release branch), do it via the versioning tool's environment variables in CI, not by editing project files.
...
Many repos assert this at test time (a fixture that parses both files and compares). Drift then fails the test suite during `dotnet test`. When you add a runtime dependency to a project that ships, update the nuspec in the same change. If the drift test fires, the nuspec is the file to update (not the test).
